...
const { emptyProgress, initDatabase, ProgressOperationError } = require("./database");
const { loadStickerCatalog, normalizeStickerCode } = require("./sticker-catalog");
const {
...
function sanitizeProgress(value) {
  ...
  return {
    ...
    adrenalyn: sanitizeFlags(value.adrenalyn || {}),
    adrenalynDuplicates: sanitizeDuplicates(value.adrenalynDuplicates || {})
  };
}
...
async function createApp(options = {}) {
  ...
  const stickerCatalog = loadStickerCatalog(appHtml);
  const inlineScript = appHtml.match(/<script>([\s\S]*?)<\/script>/)?.[1] || "";
  // HTML normalizes CRLF (and lone CR) to LF before evaluating inline scripts.
  // Hashing the normalized value keeps the CSP valid both in a Windows Docker
  // build and in Linux deployments such as Vercel.
  const normalizedInlineScript = inlineScript.replace(/\r\n?/g, "\n");
  const scriptHash = `'sha256-${createHash("sha256").update(normalizedInlineScript).digest("base64")}'`;
  ...
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'"],
        scriptSrc: ["'self'", "'wasm-unsafe-eval'", scriptHash],
        workerSrc: ["'self'"],
        mediaSrc: ["'self'", "blob:"],
  ...
  function verifyCsrf(req, res, next) {
    ...
    if (!req.session || !constantTimeTextEqual(submittedToken, req.session.csrf_token)) {
      if (req.path.startsWith("/api/")) return res.status(403).json({ error: "Solicitud inválida. Recarga la página e inténtalo nuevamente.", errorCode: "CSRF_INVALID" });
      return sendAuthPage(res, authPage({
  ...
  function requireAuth(req, res, next) {
    if (!req.session || req.session.stage !== "authenticated" || !req.user) {
      if (req.path.startsWith("/api/")) return res.status(401).json({ error: "Tu sesión expiró. Inicia sesión nuevamente antes de guardar.", errorCode: "SESSION_EXPIRED" });
  ...
  app.get("/api/progress", requireAuth, asyncRoute(async (req, res) => {
    ...
    res.json({
      progress: saved?.payload || emptyProgress(),
  ...
  app.put("/api/progress", requireAuth, verifyCsrf, asyncRoute(async (req, res) => {
    const revisionHeader = req.get("x-progress-revision");
    if (revisionHeader === undefined) {
      return res.status(428).json({ error: "Esta página necesita actualizarse antes de guardar. Recárgala para proteger tu inventario.", errorCode: "REVISION_REQUIRED" });
    }
    if (!/^(0|[1-9][0-9]*)$/.test(revisionHeader) || !Number.isSafeInteger(Number(revisionHeader))) {
      return res.status(400).json({ error: "La revisión del inventario no es válida.", errorCode: "INVALID_REVISION" });
    }
    let progress;
    try {
    ...
    const saved = await db.saveProgress(req.user.id, progress, Number(revisionHeader));
    res.json({ saved: true, updatedAt: saved.updatedAt, revision: saved.revision });
  }));

  app.post("/api/scans", requireAuth, verifyCsrf, asyncRoute(async (req, res) => {
    const { scanId, code, expectedRevision } = req.body || {};
    if (typeof scanId !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(scanId)) {
      return res.status(400).json({ error: "El identificador del escaneo no es válido.", errorCode: "INVALID_SCAN_ID" });
    }
    if (!Number.isSafeInteger(expectedRevision) || expectedRevision < 0) {
      return res.status(400).json({ error: "Actualiza tu inventario antes de confirmar el escaneo.", errorCode: "INVALID_REVISION" });
    }
    const sticker = stickerCatalog.get(normalizeStickerCode(code));
    if (!sticker) {
      return res.status(400).json({ error: "El código no existe en el catálogo de láminas. Corrígelo antes de confirmar.", errorCode: "UNKNOWN_STICKER" });
    }
    const saved = await db.recordStickerScan(req.user.id, {
      scanId: scanId.toLowerCase(), code: sticker.code, key: sticker.key, expectedRevision
    });
    res.json({ saved: true, progress: saved.payload, updatedAt: saved.updatedAt, revision: saved.revision, scan: saved.scan, replayed: saved.replayed });
  }));
  ...
  app.use((error, req, res, next) => {
    ...
    if (req.path.startsWith("/api/")) {
      if (error instanceof ProgressOperationError) {
        const snapshot = error.snapshot;
        return res.status(error.status).json({
          error: error.message,
          errorCode: error.errorCode,
          ...(snapshot ? { progress: snapshot.payload, revision: snapshot.revision, updatedAt: snapshot.updatedAt } : {})
        });
      }
      if (error.type === "entity.parse.failed" || error.type === "entity.too.large") {
        return res.status(error.status).json({ error: "El contenido de la solicitud no es válido." });
      }
      console.error("Error interno:", error.message);
      return res.status(500).json({ error: "No se pudo confirmar el guardado. Conserva este escaneo y reintenta sin iniciar otra captura.", errorCode: "SAVE_FAILED" });
    }
    console.error("Error interno:", error.message);
