...
function renderMfaSetup(csrf, qrDataUrl, secret, message = "") {
  return authPage({
    ...
    body: `${messageBox(message)}
      ...
      <p class="auth-help">La clave MFA queda cifrada en la base local y nunca se incluye en GitHub.</p>`
  });
}

async function createApp(options = {}) {
  if (!fs.existsSync(APP_PATH)) throw new Error(`No se encontró la aplicación en ${APP_PATH}`);
  const databasePath = options.databasePath || process.env.AUTH_DATABASE_PATH || path.join(__dirname, "data", "auth.sqlite");
  ...
  const authLimiter = rateLimit({
    ...
    legacyHeaders: false,
    skip: () => disableRateLimit,
    handler: (req, res) => sendAuthPage(res, authPage({
  ...
  function createSession(res, stage, userId = null) {
    const token = generateSessionToken();
    const tokenHash = hashToken(token);
    const csrfToken = generateCsrfToken();
    const ttl = stage === "authenticated" ? AUTHENTICATED_TTL : stage === "anonymous" ? ANONYMOUS_TTL : PENDING_TTL;
    const now = Date.now();
    db.prepare("INSERT INTO sessions (token_hash, user_id, stage, csrf_token, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)")
      .run(tokenHash, userId, stage, csrfToken, now + ttl, now);
  ...
  function deleteSession(req, res) {
    if (req.session?.token_hash) db.prepare("DELETE FROM sessions WHERE token_hash = ?").run(req.session.token_hash);
    res.clearCookie(COOKIE_NAME, { httpOnly: true, sameSite: "strict", secure: production, path: "/" });
  }

  function replaceSession(req, res, stage, userId = null) {
    if (req.session?.token_hash) deleteSession(req, res);
  ...
  app.use((req, res, next) => {
    ...
    const session = db.prepare("SELECT * FROM sessions WHERE token_hash = ? AND expires_at > ?").get(hashToken(rawToken), Date.now());
    if (!session) {
      res.clearCookie(COOKIE_NAME, { httpOnly: true, sameSite: "strict", secure: production, path: "/" });
      return next();
    }
    req.session = session;
    if (session.user_id) req.user = db.prepare("SELECT * FROM users WHERE id = ?").get(session.user_id);
    next();
  });

  function ensureAnonymousSession(req, res) {
    if (req.session?.stage === "anonymous") return req.session;
    return replaceSession(req, res, "anonymous");
  }

  function verifyCsrf(req, res, next) {
    if (!req.session || !constantTimeTextEqual(req.body.csrf || "", req.session.csrf_token)) {
      return sendAuthPage(res, authPage({
  ...
  app.get("/login", (req, res) => {
    ...
    const session = ensureAnonymousSession(req, res);
    ...
    if (!user || !passwordValid || locked) {
      if (user && !locked) {
        ...
        db.prepare("UPDATE users SET failed_attempts = ?, locked_until = ?, updated_at = ? WHERE id = ?")
          .run(shouldLock ? 0 : attempts, shouldLock ? now + ACCOUNT_LOCK_MS : null, now, user.id);
      }
      return sendAuthPage(res, renderLogin(req.session.csrf_token, "Credenciales incorrectas o acceso temporalmente bloqueado."));
    }
    db.prepare("UPDATE users SET failed_attempts = 0, locked_until = NULL, updated_at = ? WHERE id = ?").run(now, user.id);
    if (!user.mfa_secret) {
      const encrypted = encryptSecret(generateMfaSecret(), encryptionKey);
      db.prepare("UPDATE users SET mfa_secret = ?, updated_at = ? WHERE id = ?").run(encrypted, now, user.id);
    }
    const stage = user.mfa_enabled ? "mfa_verify" : "mfa_setup";
    replaceSession(req, res, stage, user.id);
  ...
  app.post("/mfa/setup", authLimiter, requirePending("mfa_setup"), verifyCsrf, asyncRoute(async (req, res) => {
    ...
    const activated = db.prepare("UPDATE users SET mfa_enabled = 1, last_totp_counter = ?, updated_at = ? WHERE id = ? AND last_totp_counter < ?")
      .run(counter, Date.now(), req.user.id, counter);
    if (activated.changes !== 1) return sendMfaSetupPage(res, req.session, req.user, "El código no es válido o ya fue utilizado.");
    replaceSession(req, res, "authenticated", req.user.id);
  ...
  app.post("/mfa", authLimiter, requirePending("mfa_verify"), verifyCsrf, (req, res) => {
    const secret = decryptSecret(req.user.mfa_secret, encryptionKey);
    const counter = verifyTotp(secret, req.body.code, { lastCounter: req.user.last_totp_counter });
    if (counter === null) return sendAuthPage(res, renderMfaVerify(req.session.csrf_token, "El código no es válido o ya fue utilizado."));
    const verified = db.prepare("UPDATE users SET last_totp_counter = ?, updated_at = ? WHERE id = ? AND last_totp_counter < ?")
      .run(counter, Date.now(), req.user.id, counter);
    if (verified.changes !== 1) return sendAuthPage(res, renderMfaVerify(req.session.csrf_token, "El código no es válido o ya fue utilizado."));
    replaceSession(req, res, "authenticated", req.user.id);
    res.redirect(303, "/app");
  });
